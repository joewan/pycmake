...
enum e3DSamplingType
{
	...
	_3DSMP_AO,                                                                                                                          //!< Not implemented, just presented how to use that system.
	_3DSMP_SH9,                                                                                                                         //!< Not implemented, just presented how to use that system.
};
...
class I3DSampler
{
	...
	//! Give back the "point data" array.
	f32* GetPointArray() const
	{
		return m_pPointDatas;
	}

	//! Give back the "point data" array.
	f32* GetPositionArray() const
	{
		return m_pPositions;
	}

	//! Give back the number of points.
	int GetPointNumber() const
	{
		return m_nPointNumber;
	}

	//! Set the sampling type (Just for calculation ,load care about it!).
	void SetSamplingType( const e3DSamplingType eSamplingType )
	{
		m_eSamplingType = eSamplingType;
	}

	//! Helper class to create the necessary size of array for the points.
	bool ResizePointArray( const int nPointNumber )
	{
