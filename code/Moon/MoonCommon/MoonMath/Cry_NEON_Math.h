...
//! Copies a vector from unaligned source to unaligned destination.
...
XINLINE XMVECTOR _XMLoadVector2(const float* pSource)
{
	...
	return V.vec;
				...
				#ifdef __SPU__
	return (XMVECTOR){
					 pSource[0], pSource[1], pSource[2], 0.f
	};
				#else
	XMVECTOR VL;
	XMVECTOR VR;
...
//! 3D Vector; 32 bit floating point components, keep it POD since it used in anonymous unions.
struct XMFLOAT3
{
	float x;
	float y;
...
//! 4D Vector; 32 bit floating point components, keep it POD since it used in anonymous unions.
struct XMFLOAT4
{
	float x, y, z, w;
};
...
XINLINE unsigned int XMVector3GreaterR(XMVECTOR V1, XMVECTOR V2)
{
	...
	if (_V1.v[0] > _V2.v[0] && _V1.v[1] > _V2.v[1] && _V1.v[2] > _V2.v[2])
		CR |= XM_CRMASK_CR6TRUE;
	else if (_V1.v[0] <= _V2.v[0] && _V1.v[1] <= _V2.v[1] && _V1.v[2] < _V2.v[2])
		CR |= XM_CRMASK_CR6FALSE;
	return CR;
			#else
...
XINLINE unsigned int XMVector2GreaterR(XMVECTOR V1, XMVECTOR V2)
{
	...
	if (_V1.v[0] > _V2.v[0] && _V1.v[1] > _V2.v[1])
		CR |= XM_CRMASK_CR6TRUE;
	else if (_V1.v[0] <= _V2.v[0] && _V1.v[1] <= _V2.v[1])
		CR |= XM_CRMASK_CR6FALSE;
	return CR;
			#else
	XMVECTOR dummyV1 = XMVectorSplatX(V1);
...
XINLINE XMVECTOR XMVector3Dot(XMVECTOR V1, XMVECTOR V2)
{
	...
	XMVECTOR_CONV _V2(V2);
	XMVECTOR_CONV Result;
	Result.v[0] = Result.v[1] = Result.v[2] = Result.v[3] = _V1.v[0] * _V2.v[0] + _V1.v[1] * _V2.v[1] + _V1.v[2] * _V2.v[2];
	VERIFY_XMVECTOR(Result,3)
	return Result.vec;
			#else
...
XINLINE XMVECTOR XMVector4Dot(XMVECTOR V1, XMVECTOR V2)
{
	...
	XMVECTOR_CONV _V2(V2);
	XMVECTOR_CONV Result;
	Result.v[0] = Result.v[1] = Result.v[2] = Result.v[3] = _V1.v[0] * _V2.v[0] + _V1.v[1] * _V2.v[1] + _V1.v[2] * _V2.v[2] + _V1.v[3] * _V2.v[3];
	VERIFY_XMVECTOR(Result,4)
	return Result.vec;
			#else
	XMVECTOR mul  = vec_madd(V1, V2, XMVectorZero());
...
XINLINE XMVECTOR XMVector3Cross(XMVECTOR V1, XMVECTOR V2)
{
	...
	return Result.vec;
			#else
	const vec_uchar16 perm_YZXY = (vec_uchar16)(XMVECTORI){
		_VECTORMATH_PERM_Y, _VECTORMATH_PERM_Z, _VECTORMATH_PERM_X, _VECTORMATH_PERM_Y
	};
...
XINLINE XMVECTOR XMVector2Dot(XMVECTOR V1, XMVECTOR V2)
{
	...
	XMVECTOR_CONV _V2(V2);
	XMVECTOR_CONV Result;
	Result.v[0] = Result.v[1] = Result.v[2] = Result.v[3] = _V1.v[0] * _V2.v[0] + _V1.v[1] * _V2.v[1];
