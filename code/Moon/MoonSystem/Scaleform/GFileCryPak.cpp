...
inline long GFileCryPak::SeekInternal(long offset, int origin)
{
	if (IsValidInternal())
	{
		...
		switch (origin)
		{
		case Seek_Set: newOrigin = SEEK_SET;
			break;
		case Seek_Cur: newOrigin = SEEK_CUR;
			break;
		case Seek_End: newOrigin = SEEK_END;
			break;
		}
...
inline long GFileCryStream::SeekInternal(long offset, int origin)
{
	if (IsValidInternal())
	{
		switch (origin)
		{
		case Seek_Set: m_curPos = offset;
			break;
		case Seek_Cur: m_curPos += offset;
			break;
		case Seek_End: m_curPos = m_fileSize - offset;
			break;
		}
