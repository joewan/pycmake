...
void QDrawContext::drawNumberEntry(const char* text, const Rect& rect, int flags, double sliderPos)
{
	...
	option.palette = s_lineEdit.palette();
	...
	if (sliderPos != 0.0)
	{
		...
		QColor sliderOverlayColor = interpolateColor(tree_->palette().background().color(), s_lineEdit.palette().background().color(), tree->treeStyle().sliderSaturation);
		if (pressed)
			sliderOverlayColor = s_lineEdit.palette().color(QPalette::Highlight);

		painter->setBrush(QBrush(sliderOverlayColor));
		painter->setPen(Qt::NoPen);
		painter->drawRoundedRect(sliderOverlayRect, 2, 2);

		if (tree->treeStyle().showSliderCursor && pressed) {
	...
	if (pressed)
	{
		painter->setPen(QPen(s_lineEdit.palette().color(QPalette::HighlightedText)));
	}
	else
	{
		painter->setPen(QPen(s_lineEdit.palette().color(QPalette::WindowText)));
	}
	painter->setBrush(QBrush(s_lineEdit.palette().color(QPalette::Base)));
