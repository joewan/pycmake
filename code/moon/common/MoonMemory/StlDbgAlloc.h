...
class dbg_struct_allocator:public TSimpleStructAllocator<T>
{
	...
	//! Construct default allocator (do nothing).
	dbg_struct_allocator<T>dbg_struct_allocator(const char* szParentObject, int nParentIndex = 0) :
		TSimpleStructAllocator<T>(szParentObject, nParentIndex)
	{}

	//! Construct by copying (do nothing).
	dbg_struct_allocator<T>dbg_struct_allocator(const dbg_struct_allocator<T> &rThat) :
