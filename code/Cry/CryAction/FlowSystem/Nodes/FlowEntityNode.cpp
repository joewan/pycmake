	...
	bool ReadFloatValueFromMat( IMaterial* pSubMtl, IRenderShaderResources* pRendShaderRes, const char* paramName, float &floatValue )
	{
		...
		for (int i = 0; i < shaderParams.size(); ++i)
		{
			...
			if (stricmp(param.m_Name, paramName) == 0)
			{
				...
				switch (param.m_Type)
				{
				case eType_BOOL: floatValue = param.m_Value.m_Bool;
					break;
				case eType_SHORT: floatValue = param.m_Value.m_Short;
					break;
				case eType_INT: floatValue = (float)param.m_Value.m_Int;
					break;
				case eType_FLOAT: floatValue = param.m_Value.m_Float;
					break;
				}
				return true;
			}
		...
		if (bGet)
		{
			...
			for (int i = 0; i < params.size(); ++i)
			{
				...
				if (stricmp(param.m_Name, paramNameFloat) == 0)
				{
					...
					switch (param.m_Type)
					{
					case eType_BOOL: val = param.m_Value.m_Bool;
						break;
					case eType_SHORT: val = param.m_Value.m_Short;
						break;
					case eType_INT: val = (float)param.m_Value.m_Int;
						break;
					case eType_FLOAT: val = param.m_Value.m_Float;
						break;
					default: break;
					}
