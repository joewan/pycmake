...
class CFlowNode_AIEventListener:public CFlowBaseNode<eNCT_Instanced>, IAIEventListener
{
	...
	virtual void OnAIEvent(EAIStimulusType type, const Vec3 &pos, float radius, float threat, EntityId sender)
	{
		...
		switch (type)
		{
		case AISTIM_SOUND: thr = m_thresholdSound;
			break;
		case AISTIM_COLLISION: thr = m_thresholdCollision;
			break;
		case AISTIM_BULLET_HIT: thr = m_thresholdBullet;
			break;
		case AISTIM_EXPLOSION: thr = m_thresholdExplosion;
			break;
		default: return;
		}
		...
		if (thr > 0.0f && u > thr)
		{
			...
			switch (type)
			{
			case AISTIM_SOUND: i = 0;
				break;
			case AISTIM_COLLISION: i = 1;
				break;
			case AISTIM_BULLET_HIT: i = 2;
				break;
			case AISTIM_EXPLOSION: i = 3;
				break;
			default: return;
			}
