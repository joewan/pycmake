		...
		if (event == eFE_Activate && IsPortActive(pActInfo,IN_SET) && pActInfo->pEntity && pActInfo->pEntity->GetPhysics())
		{
			...
			if (pEntityTable->GetValue("SetPhysicParams", SetParams) && SetParams)
			{
				...
				for (int i = 0; i < 10; i++)
				{
					...
					{
					case eFDT_Int: pParams->SetValue(name, GetPortInt(pActInfo,IN_VAL0 + i * 2));
						break;
					case eFDT_Float: pParams->SetValue(name, GetPortFloat(pActInfo,IN_VAL0 + i * 2));
						break;
					case eFDT_EntityId: pParams->SetValue(name, GetPortEntityId(pActInfo,IN_VAL0 + i * 2));
						break;
					case eFDT_Vec3: pParams->SetValue(name, GetPortVec3(pActInfo,IN_VAL0 + i * 2));
						break;
					case eFDT_Bool: pParams->SetValue(name, GetPortBool(pActInfo,IN_VAL0 + i * 2) ? 1 : 0);
						break;
					case eFDT_String: pParams->SetValue(name, GetPortString(pActInfo,IN_VAL0 + i * 2).c_str());
						break;
					}
				}
