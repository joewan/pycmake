...
void CPhysRenderer::DrawGeometry(IGeometry* pGeom, geom_world_data* pgwd, int idxColor, int bSlowFadein, const Vec3 &sweepDir)
{
	...
	else
	{
		...
		else
		{
			...
			case GEOM_BOX: *(primitives::box*)m_geomBuf[m_iLastGeom].buf = *(primitives::box*)pGeom->GetData();
				break;
			case GEOM_SPHERE: *(primitives::sphere*)m_geomBuf[m_iLastGeom].buf = *(primitives::sphere*)pGeom->GetData();
				break;
			case GEOM_CYLINDER: case GEOM_CAPSULE: *(primitives::cylinder*)m_geomBuf[m_iLastGeom].buf = *(primitives::cylinder*)pGeom->GetData();
				break;
			}
			if (!pgwd)
...
void CPhysRenderer::DrawFrame(const Vec3 &pnt, const Vec3* axes, const float scale, const Vec3* limits, const int axes_locked)
{
	...
	float              fclr[4][4] = {{1.f,0.f,0.f,1.f},{0.f,1.f,0.f,1.f},{0.f,0.f,1.f,1.f},{1.f,1.f,1.f,1.f}};
	char               str[128];
