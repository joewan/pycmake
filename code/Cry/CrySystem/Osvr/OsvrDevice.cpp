...
namespace Osvr
{
static const char* clientName = "Cryapp";
...
bool Device::InitializeDevice()
{
	...
	m_context = osvrClientInit(clientName, 0);
...
void Device::UpdateInternal(EInternalUpdate e)
{}
...
void Device::UpdateTrackingState(EVRComponent e)
{
	...
	if ((e & eVRComponent_Hmd) != 0)
	{
		...
		/*
		   //fetch the current pose and update the current pose
		   OSVR_TimeValue timestamp;
		   OSVR_PoseState poseState;
		   osvrGetPoseState(m_headInterface, &timestamp, &poseState);
		   UpdatePose(poseState);
		 */
...
bool Device::initializeRenderer(void* d3dDevice, void* d3dContext)
{
	...
	m_renderContext = osvrClientInit(clientName, 0);
	...
	bool success = osvrCreateRenderManagerD3D11(m_renderContext, "Direct3D11", library, &m_renderManager, &m_renderManagerD3D11) == OSVR_RETURN_SUCCESS;
	if (success && m_renderManager && m_renderManagerD3D11)
	{
		OSVR_OpenResultsD3D11 res;
		success = osvrRenderManagerOpenDisplayD3D11(m_renderManagerD3D11, &res) == OSVR_RETURN_SUCCESS;
	}
	if (success)
	{
		success = osvrRenderManagerGetDoingOkay(m_renderManager) == OSVR_RETURN_SUCCESS;
	}
...
bool Device::presentTextureSet(int textureSetIndex)
{
	...
	osvrRenderManagerFinishPresentRenderBuffers(m_renderManager, presentBufState, m_renderParams, OSVR_FALSE);
...
bool Device::registerTextureSwapSet(TextureSwapSet* swapSet)
{
	...
	for (int i = 0; i < swapSet->numTextureSets; ++i)
	{
		...
		TextureSet &texSet   = swapSet->textureSets[i];
		...
		for (int texIndex = 0; texIndex < texCount; ++texIndex)
		{
			...
			renderBuffer.colorBuffer        = static_cast<ID3D11Texture2D*>(texSet.textures[texIndex].texture);
			renderBuffer.colorBufferView    = static_cast<ID3D11RenderTargetView*>(texSet.textures[texIndex].rtView);
			renderBuffer.depthStencilBuffer = NULL;
			renderBuffer.depthStencilView   = NULL;

			renderBuffers.Add(renderBuffer);

			success = success && osvrRenderManagerRegisterRenderBufferD3D11(regBufState, renderBuffer) == OSVR_RETURN_SUCCESS;
		}

	}


	success = success && osvrRenderManagerFinishRegisterRenderBuffers(m_renderManagerD3D11, regBufState, OSVR_TRUE) == OSVR_RETURN_SUCCESS;
