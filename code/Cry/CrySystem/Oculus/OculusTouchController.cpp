...
//
// Description: Oculus Touch Controller functionality updated to Oculus SDK 0.7.0
// - 10/14/2015   Created by Dario Sancho
...
void COculusController::Update()
{
	...
	if (m_pSession)
	{
		...
		if (bHasInputState = (ovr_GetInputState(m_pSession, ovrControllerType_Touch, &inputState) == ovrSuccess))
		{
			...
			// connection
			m_state.m_connected[eHmdController_OculusLeftHand]  = (inputState.ConnectedControllerTypes & ovrControllerType_LTouch) != 0;
			m_state.m_connected[eHmdController_OculusRightHand] = (inputState.ConnectedControllerTypes & ovrControllerType_RTouch) != 0;
