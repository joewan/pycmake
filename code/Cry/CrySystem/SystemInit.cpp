...
#include <CryExtension/CryCreateClassInstance.h>
	...
	#include "Scaleform/SharedResources.h"
	#include <GFxVideo.h>
#endif
...
int StrToPhysHelpers(const char* strHelpers)
{
	...
	for (ptr = strHelpers; *ptr && *ptr != '_'; ptr++)
	{
		...
		case 'r': iHelpers |= 512;
			break;
		case 'R': iHelpers |= 1024;
			break;
		case 'l': iHelpers |= 2048;
			break;
		case 'i': iHelpers |= 4096;
			break;
		case 'e': iHelpers |= 8192;
			break;
		case 'g': iHelpers |= 16384;
			break;
		case 'w': iHelpers |= 32768;
			break;
		case 'a': iHelpers |= 32;
			break;
		case 'y': iHelpers |= 64;
...
//#endif
	...
	if (startupParams.pLastPLMEvent)
	{
		...
		case EPLMEV_ON_CONSTRAINED: m_env.ePLM_State = EPLM_CONSTRAINED;
			break;
		case EPLMEV_ON_SUSPENDING: m_env.ePLM_State = EPLM_SUSPENDED;
			break;
		}
	}
...
void CSystem::CreateSystemVars()
{
	...
	m_sys_dll_response_system = REGISTER_STRING("sys_dll_response_system", "CryDynamicResponseSystem.dll", dllFlags, "Specifies the DLL to load for the dynamic response system");
