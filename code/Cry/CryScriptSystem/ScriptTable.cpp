...
static void IterTable(lua_State* L, IScriptTableDumpSink* p)
{
	...
	while (lua_next(L, -2))
	{
		...
		if (keyType == LUA_TSTRING)
		{
			...
			switch (valType)
			{
			case LUA_TNIL: p->OnElementFound(key, svtNull);
				break;
			case LUA_TBOOLEAN: p->OnElementFound(key, svtBool);
				break;
			case LUA_TLIGHTUSERDATA: p->OnElementFound(key, svtPointer);
				break;
			case LUA_TNUMBER: p->OnElementFound(key, svtNumber);
				break;
			case LUA_TSTRING: p->OnElementFound(key, svtString);
				break;
			case LUA_TTABLE:
				if (strcmp(key, "__index") != 0)
					p->OnElementFound(key, svtObject);
				break;
			case LUA_TFUNCTION: p->OnElementFound(key, svtFunction);
				break;
			case LUA_TUSERDATA: p->OnElementFound(key, svtUserData);
				break;
			}
		}
		else
		{
			...
			switch (valType)
			{
			case LUA_TNIL: p->OnElementFound(idx, svtNull);
				break;
			case LUA_TBOOLEAN: p->OnElementFound(idx, svtBool);
				break;
			case LUA_TLIGHTUSERDATA: p->OnElementFound(idx, svtPointer);
				break;
			case LUA_TNUMBER: p->OnElementFound(idx, svtNumber);
				break;
			case LUA_TSTRING: p->OnElementFound(idx, svtString);
				break;
			case LUA_TTABLE: p->OnElementFound(idx, svtObject);
				break;
			case LUA_TFUNCTION: p->OnElementFound(idx, svtFunction);
				break;
			case LUA_TUSERDATA: p->OnElementFound(idx, svtUserData);
				break;
			}
		}
...
void CScriptTable::Dump(IScriptTableDumpSink* p)
{
	...
	while (lua_next(L, trgTable) != 0)
	{
		...
		if (lua_type(L, -2) == LUA_TSTRING)
		{
			...
			switch (lua_type(L,-1))
			{
			case LUA_TNIL: p->OnElementFound(sName, svtNull);
				break;
			case LUA_TBOOLEAN: p->OnElementFound(sName, svtBool);
				break;
			case LUA_TLIGHTUSERDATA: p->OnElementFound(sName, svtPointer);
				break;
			case LUA_TNUMBER: p->OnElementFound(sName, svtNumber);
				break;
			case LUA_TSTRING: p->OnElementFound(sName, svtString);
				break;
			case LUA_TTABLE:
				if (strcmp(sName, "__index") != 0)
					p->OnElementFound(sName, svtObject);
				break;
			case LUA_TFUNCTION: p->OnElementFound(sName, svtFunction);
				break;
			case LUA_TUSERDATA: p->OnElementFound(sName, svtUserData);
				break;
			}
		}
		else
		{
			...
			switch (lua_type(L,-1))
			{
			case LUA_TNIL: p->OnElementFound(nIdx, svtNull);
				break;
			case LUA_TBOOLEAN: p->OnElementFound(nIdx, svtBool);
				break;
			case LUA_TLIGHTUSERDATA: p->OnElementFound(nIdx, svtPointer);
				break;
			case LUA_TNUMBER: p->OnElementFound(nIdx, svtNumber);
				break;
			case LUA_TSTRING: p->OnElementFound(nIdx, svtString);
				break;
			case LUA_TTABLE: p->OnElementFound(nIdx, svtObject);
				break;
			case LUA_TFUNCTION: p->OnElementFound(nIdx, svtFunction);
				break;
			case LUA_TUSERDATA: p->OnElementFound(nIdx, svtUserData);
				break;
			}
		}
...
ScriptVarType CScriptTable::GetValueType( const char* sKey )
{
	...
	switch (luatype)
	{
	case LUA_TNIL: type = svtNull;
		break;
	case LUA_TBOOLEAN: type = svtBool;
		break;
	case LUA_TNUMBER: type = svtNumber;
		break;
	case LUA_TSTRING: type = svtString;
		break;
	case LUA_TFUNCTION: type = svtFunction;
		break;
	case LUA_TLIGHTUSERDATA: type = svtPointer;
		break;
	case LUA_TTABLE: type = svtObject;
		break;
	}
