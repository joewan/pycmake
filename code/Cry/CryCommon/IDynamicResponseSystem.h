...
struct ISpeakerManagerLipsyncProvider
{
	virtual int  OnLineStarted(IResponseActor* pSpeaker, const IDialogLine* pLine)              = 0; //the value that is returned here is the lipsyncId
	virtual void OnLineEnded(int lipsyncId, IResponseActor* pSpeaker, const IDialogLine* pLine) = 0;
	virtual bool Update(int lipsyncId, IResponseActor* pSpeaker, const IDialogLine* pLine)      = 0; //returns if the lip animation has finished
	//TODO: also pass in the current audio data for 'Update'
};

struct ISpeakerManagerListener
{
	enum eLineEvent
	{
...
{
	virtual bool IsSpeaking(const IResponseActor* pActor, const CHashedString &lineID = CHashedString::GetEmptyHashedString()) const = 0;
	...
	virtual bool AddListener(ISpeakerManagerListener* pListener)                                                                     = 0;
	virtual bool RemoveListener(ISpeakerManagerListener* pListener)                                                                  = 0;
	virtual void SetCustomLipsyncProvider(ISpeakerManagerLipsyncProvider* pProvider)                                                 = 0;
};
...
struct IResponseActor
{
	 ...
	 * @param delayBeforeFiring - time that the signal is queued before processed
	 * @see IDynamicResponseSystem::CreateContextCollection, IDynamicResponseSystem::CancelSignalProcessing
	 */
	virtual void QueueSignal(const CHashedString &signalName, IVariableCollectionSharedPtr pSignalContext = nullptr, float delayBeforeFiring = 0.0f) = 0;
};
...
struct IDynamicResponseSystem
{
	 ...
	 * @param delayBeforeCancel - delay before we stop the execution
	 */
	virtual void CancelSignalProcessing(const CHashedString &signalName, IResponseActor* pSender = nullptr, float delayBeforeCancel = 0.0f) = 0;
	...
	enum eResetHints
	{
		...
		eResetHint_Responses  = BIT(0),
		eResetHint_Variables  = BIT(1),
		eResetHint_Speaker    = BIT(2),
		eResetHint_DebugInfos = BIT(3),
